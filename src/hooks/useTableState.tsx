...
export const useTableState = ({ 
  ...
  // Handle column pinning changes - FIXED
  const handleColumnPinningChange = useCallback((updater: any) => {
    if (typeof updater === 'function') {
      // Convert current pinning to TanStack format
      const currentTanStackPinning = Object.entries(columnPinning).reduce((acc, [key, value]) => {
        if (value !== false) {
          acc[key] = value;
        }
        return acc;
      ...
      Object.entries(newTanStackPinning).forEach(([key, value]) => {
        if (value === 'left' || value === 'right') {
          newColumnPinning[key] = value;
        }
      });
  ...
  const tablePinningState = useMemo(() => {
    ...
    Object.entries(columnPinning).forEach(([key, value]) => {
      if (value === 'left') {
        leftPinned.push(key);
      } else if (value === 'right') {
        rightPinned.push(key);
      }
    });
